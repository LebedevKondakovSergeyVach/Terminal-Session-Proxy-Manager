...
> [!NOTE]
> Фоновый процесс не может изменять переменные окружения родительской оболочки. Именно поэтому прямой запуск бинарника не может обновить вашу текущую сессию.

Чтобы решить эту проблему, shell-функция `proxy` является обёрткой над бинарником:
