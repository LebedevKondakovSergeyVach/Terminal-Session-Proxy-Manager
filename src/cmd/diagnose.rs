...
pub async fn run_diagnose(config: &AppConfig) -> Result<()> {
    ...
    println!("Активный профиль : {} ({})", name.yellow().bold(), config.active_profile);
    ...
    if let Ok(addr) = socket_addr.parse() {
        if TcpStream::connect_timeout(&addr, Duration::from_secs(2)).is_ok() {
            ...
            println!("{}", format!("❌ Порт закрыт! Проверьте клиент {}", name).red().bold());
        }
    } else {
    ...
    println!("   • https_proxy = {}", env::var("https_proxy").unwrap_or_else(|_| "<не задан>".to_string()));
    ...
    for ep in &config.diagnose_endpoints {
        ...
        match client.get(&ep.url).send().await {
            Ok(resp) => println!("{}", format!("✅ Доступен (HTTP {})", resp.status().as_u16()).green().bold()),
